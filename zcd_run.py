...
#Add module sources.
zcd_src_lib = ui.add_library("src_lib")
zcd_src_lib.add_source_files("zcd.vhd")

#Add tb sources.
zcd_tb_lib = ui.add_library("tb_lib")
zcd_tb_lib.add_source_files("tb_zcd.vhd")
...
if(code_coverage==True):
  zcd_src_lib.add_compile_option   ("ghdl.flags"     , [  "-fprofile-arcs","-ftest-coverage" ])
  zcd_tb_lib.add_compile_option("ghdl.flags"     , [  "-fprofile-arcs","-ftest-coverage" ])
...
if all_ok:
  if(code_coverage==True):
    subprocess.call(["lcov", "--capture", "--directory", "zcd.gcda", "--output-file",  "code_0.info" ])
